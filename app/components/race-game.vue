...
<template>
  <div class="p-2 md:p-3 rounded-xl">
    ...
    <Transition enter-active-class="transition-all duration-500" enter-from-class="opacity-0 scale-50" enter-to-class="opacity-100 scale-100">
      <div v-if="winner" class="text-center items-center justify-center mb-2 p-1">
        <div class="flex items-center justify-center gap-2">
          <UIcon :name="winner.icon" class="size-5 md:size-6" />
          <!-- CHANGED: responsive text size -->
          <p class="text-sm md:text-base font-bold">{{ winner.name }} Wins! 🎉</p>
        </div>
      </div>
    </Transition>

    <div class="flex flex-col gap-2 mb-3">
      <div v-for="duck in ducks" :key="duck.id" class="flex items-center gap-1 md:gap-2">

        <!-- CHANGED: responsive name width and text -->
        <span class="text-xs md:text-sm w-16 md:w-20 shrink-0 text-right">{{ duck.name }}</span>

        <div class="flex-1 relative h-7 md:h-8 bg-white/5 rounded-full overflow-hidden border border-white/10 shadow-sm">
          ...
          <div class="absolute top-0 bottom-0 flex items-center transition-all duration-100 ease-linear"
            ...
            <UIcon :name="duck.icon" class="size-5 md:size-6" />
          </div>
        </div>
    ...
    <div class="flex justify-center gap-3 md:gap-4">
      <button @click="startRace" :disabled="raceStarted"
        class="px-3 py-1.5 md:px-4 md:py-2 bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl font-bold cursor-pointer transition-colors duration-200 border-2 uppercase text-xs md:text-sm shadow-md">
        Start Race
      </button>
      <button @click="resetRace"
        class="px-3 py-1.5 md:px-4 md:py-2 rounded-xl font-bold cursor-pointer transition-colors duration-200 bg-blue-500 border-2 uppercase hover:bg-blue-600 text-xs md:text-sm shadow-md">
        Reset
      </button>
    </div>
  </div>
